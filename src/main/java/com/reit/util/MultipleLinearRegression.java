...
/**
 * The {@code MultipleLinearRegression} class performs a multiple linear
 * regression on an set of <em>N</em> data points using the model <em>y</em> =
 * &beta;<sub>0</sub> + &beta;<sub>1</sub> <em>x</em><sub>1</sub> + ... +
 * &beta;<sub><em>p</em></sub> <em>x<sub>p</sub></em>, where <em>y</em> is the
 * response (or dependent) variable, and <em>x</em><sub>1</sub>,
 * <em>x</em><sub>2</sub>, ..., <em>x<sub>p</sub></em> are the <em>p</em>
 * predictor (or independent) variables. The parameters
 * &beta;<sub><em>i</em></sub> are chosen to minimize the sum of squared
 * residuals of the multiple linear regression model. It also computes the
...
public class MultipleLinearRegression {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < featureSize; i++) {
			System.out.println(regression.beta(i));
...
//				regression.beta(0) + ":::" + regression.beta(1) + ":::" + regression.beta(2) + ":::" + regression.R2());
		// System.out.println(
		// regression.beta(0) * 1 + regression.beta(1) * 20 + regression.beta(2) * 41 +
		// regression.beta(3) * 50);
	}
}
